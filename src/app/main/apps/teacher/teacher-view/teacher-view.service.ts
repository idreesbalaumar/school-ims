...
@Injectable()
export class TeacherViewService implements Resolve<any> {
  public rows: any;
  public onTeacherViewChanged: BehaviorSubject<any>;
  public id;
  ...
  constructor(private _httpClient: HttpClient) {
    // Set the defaults
    this.onTeacherViewChanged = new BehaviorSubject({});
  ...
  getApiData(id: number): Promise<any[]> {
    ...
    return new Promise((resolve, reject) => {
      this._httpClient.get(url).subscribe((response: any) => {
        this.rows = response;
        this.onTeacherViewChanged.next(this.rows);
        resolve(this.rows);
      }, reject);
    });
  }
